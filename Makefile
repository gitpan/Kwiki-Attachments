# This Makefile is for the Kwiki::Attachments extension to perl.
#
...
#     ABSTRACT => q[Kwiki File Attachments Plugin]
#     AUTHOR => q[Sue Spence <sue_cpan@pennine.com>]
#     DIR => []
#     DISTNAME => q[Kwiki-Attachments]
#     NAME => q[Kwiki::Attachments]
#     NO_META => q[1]
#     PL_FILES => {  }
#     PREREQ_PM => { Kwiki=>q[0.37] }
#     VERSION => q[0.11]
...
H_FILES  = 
MAN1PODS = 
MAN3PODS = lib/Kwiki/Attachments.pm

# Where is the Config information that we are using/depend on
CONFIGDEP = $(PERL_ARCHLIB)$(DIRFILESEP)Config.pm $(PERL_INC)$(DIRFILESEP)config.h

# Where to build things
INST_LIBDIR      = $(INST_LIB)/Kwiki
INST_ARCHLIBDIR  = $(INST_ARCHLIB)/Kwiki
...
TO_INST_PM = lib/Kwiki/Attachments.pm

PM_TO_BLIB = lib/Kwiki/Attachments.pm \
...
DISTNAME = Kwiki-Attachments
DISTVNAME = Kwiki-Attachments-0.11
...
manifypods : pure_all  \
	lib/Kwiki/Attachments.pm \
	lib/Kwiki/Attachments.pm
	$(NOECHO) $(POD2MAN) --section=3pm --perm_rw=$(PERM_RW)\
	  lib/Kwiki/Attachments.pm $(INST_MAN3DIR)/Kwiki::Attachments.$(MAN3EXT) 
...
realclean purge ::  clean realclean_subdirs
	...
	$(RM_RF) $(DISTVNAME)
	$(RM_F)  blib/lib/Kwiki/Attachments.pm $(MAKEFILE_OLD) $(FIRST_MAKEFILE)
...
ppd:
	...
	$(NOECHO) $(ECHO) '    <ABSTRACT>Kwiki File Attachments Plugin</ABSTRACT>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>Sue Spence &lt;sue_cpan@pennine.com&gt;</AUTHOR>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="Kwiki" VERSION="0,37,0,0" />' >> $(DISTNAME).ppd
...
pm_to_blib: $(TO_INST_PM)
	$(NOECHO) $(PERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', '\''$(PM_FILTER)'\'')'\
	  lib/Kwiki/Attachments.pm blib/lib/Kwiki/Attachments.pm 
	$(NOECHO) $(TOUCH) $@
